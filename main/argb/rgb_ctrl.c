...
static void argb_ctrl_task(void* arg)
{
    ...
    while (1) {
        ...
        if (lastSw != g_channel1_sw) {
            lastSw = g_channel1_sw;
            if (g_channel1_sw) {
                ESP_ERROR_CHECK(rmt_enable(g_led_chan1));
            } else {
        ...
        if (g_channel1_sw) {
            ...
            for (int j = 0; j < EXAMPLE_LED_NUMBERS; j++) {
                // Build RGB pixels
                // hue = j * 10 + start_rgb;
                // uint16_t v = (rand() % 21) + 80;
                uint16_t v = 40;
                led_strip_hsv2rgb(340, 100, v, &red, &green, &blue);
                g_led_strip_pixels[j * 3] = green;
                g_led_strip_pixels[j * 3 + 1] = red;
                g_led_strip_pixels[j * 3 + 2] = blue;
            }
